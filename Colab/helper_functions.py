...
"GradCam Source: https://keras.io/examples/vision/grad_cam/"
...
def display_sample_with_heatmap(test_df, predicted, rd_index, model,
                                preprocess, conv_name=None, alpha=0.6, title="Title", incorrect=False,
                                fontsize=20, fontname="OPTITimes-Roman", paths="/img"):
    plt.rcParams['font.family'] = fontname
    num_images = len(rd_index)
    fig, axes = plt.subplots(nrows=num_images, ncols=3, figsize=(15, 5 * num_images),
                             subplot_kw={'xticks': [], 'yticks': []})
    if incorrect:
        sub_title = " Incorrect Predict"
    else:
        sub_title = " Correct Predict"

    fig.suptitle(title+sub_title, fontsize=fontsize+2, fontweight='bold', y=1.0)
    ...
    for i, idx in enumerate(rd_index):
        img_path = test_df.Filepath.iloc[idx]

        original_img = keras.utils.load_img(img_path)
        original_img_array = keras.utils.img_to_array(original_img)
        original_size = (original_img_array.shape[1], original_img_array.shape[0])

        img_array = get_img_array(img_path, size=(224, 224))
        img_array = preprocess(img_array)

        # Ensure conv_name is not None, use default layer if needed
        if conv_name is None:
            conv_name = model.layers[-2].name

        # Remove activation function in last layers
        temp_model = clone_model(model)
        temp_model.layers[-1].activation = None

        # Create Grad-CAM heatmap
        heatmap = make_gradcam_heatmap(img_array, model, conv_name)
        heatmap_resized = cv2.resize(heatmap, original_size, interpolation=cv2.INTER_CUBIC)

        # Generate the heatmap-overlayed image
        superimposed_img = display_gradcam(img_path, heatmap, alpha)

        if isinstance(test_df.Label.iloc[idx], str):
            pred_color = 'green' if str(test_df.Label.iloc[idx]) == str(predicted[idx]) else 'red'
        else:
            pred_color = 'green' if test_df.Label.iloc[idx] == predicted[idx] else 'red'

        # Plot Original Image
        axes[i, 0].imshow(keras.utils.load_img(img_path))
        axes[i, 0].set_title(f"True: {test_df.Label.iloc[idx]}\nPredicted: {predicted[idx]}", color=pred_color, fontsize=fontsize-2)

        # Plot Heatmap
        axes[i, 1].imshow(heatmap_resized)
        axes[i, 1].set_title("Heatmap", fontsize=fontsize-2)

        # Plot Original + Heatmap
        axes[i, 2].imshow(superimposed_img)
        axes[i, 2].set_title("Original + Heatmap", fontsize=fontsize-2)

    plt.tight_layout()

    if incorrect:
        plt.savefig(f"{paths}/Incorrect_Predict", bbox_inches='tight', format='png', dpi=300)
    else:
        plt.savefig(f"{paths}/Correct_Predict", bbox_inches='tight', format='png', dpi=300)
    plt.show()
